...
function mdl_click_callback() {
	...
	console.log("First offset: 0x%s", first_ofs);
	console.log("Difference: %d", first_ofs - (ofs + 4));
	var anims = [];
...
function draw_texture(canvas, texture, list) {

	async.eachSeries(list, function (img, nextImg) {
		...
		db.files.where(
			...
			img.pallet_file
		).toArray().then(function (file) {
			if (!file.length) {
				throw new Error(img);
			}

			var pallet_file = file[1] || file[0];
			var image_file = file[0];
		...
		/*
		var div = document.getElementById("mdl_list");
